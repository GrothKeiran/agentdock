...
function Invoke-TestSetup([string] $Path, [string] $Label) {
    ...
    $process = Start-Process -FilePath $Path -ArgumentList @('/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART', "/DIR=`"$root`"", "/LOG=`"$log`"", '/MODE=local', '/AUTOSTART=1', '/ADMINMODE=standard') -PassThru
