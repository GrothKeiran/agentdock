...
func TestWindowsInstallerWorkflowHasAlwaysPresentPullRequestGate(t *testing.T) {
	...
	for _, want := range []string{
		...
		"needs: [changes, validate, official-upgrade, published-validation]",
		"PUBLISHED_RESULT: ${{ needs.published-validation.result }}",
		"OFFICIAL_RESULT: ${{ needs.official-upgrade.result }}",
		"if: always()",
